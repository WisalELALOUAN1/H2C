...
const ManagerPendingRequests: React.FC = () => {
  ...
  const fetchPendingRequests = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/gestion-absences-conges/manager/demandes-attente/', {
  ...
  const handleStatusChange = async (demandeId: number, newStatus: 'validé' | 'refusé') => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/gestion-absences-conges/manager/valider-demande/${demandeId}/`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        notification.innerHTML = `
          <div class="flex items-center space-x-2">
            ...
            <span>Demande ${newStatus} avec succès</span>
          </div>
        `;
        ...
        <div className="mb-8">
          ...
          <div className="mt-6 flex items-center space-x-6">
            <div className="flex items-center space-x-2 text-amber-800">
              ...
              <span className="font-medium">{demandes.length} demande{demandes.length > 1 ? 's' : ''} en attente</span>
            </div>
          </div>
        </div>
                ...
                <tbody>
                  {demandes.map((demande, index) => (
                    ...
                    >
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-amber-200 to-orange-200 rounded-full flex items-center justify-center">
                            <span className="text-amber-800 font-semibold text-sm">
                              {demande.user.split(' ').map(n => n[0]).join('')}
                            </span>
                          </div>
                          <div>
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getTypeColor(demande.type_demande)}`}>
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center space-x-2">
                          <button
                            ...
                            className="flex items-center space-x-1 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg"
                          >
                          ...
                          <button
                            ...
                            disabled={processingId === demande.id}
                            className="flex items-center space-x-1 px-4 py-2 bg-rose-500 text-white rounded-lg hover:bg-rose-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg"
                          >
                            {processingId === demande.id ? (
                              <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                            ) : (
