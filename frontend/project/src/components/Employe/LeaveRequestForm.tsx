...
import { createLeaveRequestApi, updateLeaveRequestApi } from "../../services/api"
...
const LeaveRequestForm: React.FC<Props> = ({ onClose, onSuccess, existingRequest, maxNegatif }) => {
  ...
  useEffect(() => {
    ...
    const fetchSolde = async () => {
      ...
      try {
        const response = await fetch('http://localhost:8000/gestion-absences-conges/employe/dashboard/', {
  ...
  const validateForm = () => {
    ...
    if (form.commentaire.trim().length < 10) errors.push("Le commentaire doit contenir au moins 10 caractères")
    ...
    if (form.date_debut && form.date_fin) {
      ...
      if (form.type_demande === "payé") {
        const daysRequested = calculateDays(form.date_debut, form.date_fin, form.demi_jour);
        const newSolde = currentSolde - daysRequested;
        console.log("Current Solde:", currentSolde, "Days Requested:", daysRequested, "New Solde:", newSolde);
        console.log("Max Negatif:", maxNegatif);
        if (newSolde < maxNegatif) {
          errors.push(
            `Solde insuffisant. Vous demandez ${daysRequested} jours alors que votre solde est de ${currentSolde} jours. ` +
            `Le solde ne peut pas descendre en dessous de ${maxNegatif} jours.`
          );
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingRequest) {
        await updateLeaveRequestApi(existingRequest.id, payload)
      } else {
        await createLeaveRequestApi(payload)
      }
      ...
      if (errorMessage.includes(":")) {
        ...
        setError(errorMessages.length > 0 ? errorMessages : [errorMessage])
      } else {
        ...
        <div className="p-8 space-y-6">
              ...
              <div className="relative">
                ...
                <div
                  className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all duration-200 ${
                    form.demi_jour ? "bg-amber-600 border-amber-600" : "border-stone-300 hover:border-amber-400"
                  }`}
                >
                  {form.demi_jour && (
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <p className="text-xs text-stone-400">{form.commentaire.length} caractères</p>
            </div>
          </div>
